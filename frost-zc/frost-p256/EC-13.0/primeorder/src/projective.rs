...
use elliptic_curve::{
    ...
    group::{
        self, cofactor::CofactorGroup, prime::{PrimeCurve, PrimeGroup}, Curve, Group, GroupEncoding
    },
...
impl<C> common_traits::ByteCode for ProjectivePoint<C> where C:PrimeCurveParams {
    fn get_unique_byte_array(&self) -> Vec<u8> {
        self.to_affine().x().to_vec()
    }
} 
